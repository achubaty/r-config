...
4. Install:
    ...
    ```bash
    cd ~/R/src/R-${R_VERSION}
    ./configure --prefix=/usr/local/lib/R-${R_VERSION} --enable-R-shlib --with-blas --with-lapack
    make
    sudo make install
    ```
...
5. Create symlink:
    ...
    ```bash
    sudo ln -s /usr/local/lib/R-${R_VERSION}/bin/R /usr/local/bin/R-${R_VERSION}
    ```
